...
function ProductsList({ products }: { products: Product[] }) {
  return (
    <div className="mt-12 grid gap-y-8">
      {products.map((item) => {
        const { name, id: productId, price, image, company } = item;
              ...
              <Card className="transform group-hover:shadow-xl transition-shadow duration-300 bg-primary-foreground">
                <CardContent className="p-8 gap-y-4 grid md:grid-cols-3">
                  ...
                  <div>
                    ...
                    <h4 className="text-muted-foreground text-center">{company}</h4>
                  </div>
                  <p className="text-accent-foreground text-lg md:ml-auto">
                    {dollarsAmount}
                  </p>
                </CardContent>
              </Card>
